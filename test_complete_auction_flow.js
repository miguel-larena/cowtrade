...
async function testCompleteAuctionFlow() {
  try {
    console.log('🧪 Testing Complete Auction Flow...\n');
    console.log('Scenario: Player 1 has 1x $10 + 1x $50 = $60 total');
    ...
    // Step 7: Wait for match bid phase to timeout (simulate by calling endAuction again)
    console.log('7. Waiting for match bid phase to timeout...');
    
    // Wait 16 seconds for the match bid phase to timeout
    console.log('   Waiting 16 seconds for timeout...');
    await new Promise(resolve => setTimeout(resolve, 16000));
    
    const timeoutResponse = await fetch(`${API_BASE}/${gameId}/auction/end`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' }
    });
    const finalGame = await timeoutResponse.json();
    
    console.log(`✅ Match bid phase timed out`);
